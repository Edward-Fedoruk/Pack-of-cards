...
const animate = i => {
  setStyle(
    "transform", 
    `translate(${posX - parseInt(takeElem(".card")[i].style.zIndex) / 3}px, 
              ${posY - parseInt(takeElem(".card")[i].style.zIndex) / 3}px)`, 
    takeElem(".card")[i]);
...
const animateShuffle = (i) => {
  if(i % 2 === 0) {
    setStyle(
      "transform", 
      `translate(${posX + 40 - takeElem(".card")[i].style.zIndex / 2}px, 
                ${posY - takeElem(".card")[i].style.zIndex / 2}px)`,
      takeElem(".card")[i]);
  }
  else if(i % 2 !== 0) {
    setStyle(
...
const memoizeRand = () => {
  ...
  return function $memoize(max) {
    ...
    cash.includes(rnd) ? $memoize(max) : cash.push(rnd);
